...
- (void)takePhoto
{
    [self.stillCamera capturePhotoAsImageProcessedUpToFilter:self.currentFilter withCompletionHandler:^(UIImage *processedImage, NSError *error){
        if (!error) {
            // 写真をカメラロールに保存
